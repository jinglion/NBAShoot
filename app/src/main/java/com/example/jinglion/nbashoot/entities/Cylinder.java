...
public class Cylinder {//支架
    ...
    public Cylinder(float length, float circle_radius, float degreespan, int col, int textureid){
        ...
        int spannum = (int)(360.0f/degreespan);//圆截环的份数
        ...
        for (float circle_degree=360.0f;circle_degree>0.0f;circle_degree-=degreespan){
            for (int j=0;j<col;j++){
                ...
                float y1 = (float)(circle_radius*Math.sin(Math.sin(Math.toRadians(circle_degree))));
                float z1 = (float) (circle_radius*Math.cos(Math.toRadians(Math.toRadians(circle_degree))));
